...
service Terminal {
  ...
  rpc CancelEvent(TerminalCancelEvent) returns (control.InvokeResponse);
...
message Command {
  string id = 1;
  int64 timestamp = 2;
  map<string, Parameter> payload = 3;
}
```

这里我想多说几句 `Parameter` 的设计。对于动态参数，常见的做法是使用 `google.protobuf.Any` 配合 gRPC 反射插件，这样任何消息类型都可以直接传递，灵活性很高。但在这个系统里，我更希望控制终端在注册阶段就把参数结构声明清楚，这样控制系统不需要依赖运行时反射，就能根据 `ParameterType` 直接生成命令表单、校验输入、展示状态面板。所以我用了一个强类型的 oneof 结构，`Command` 的 `payload` 和 `TerminalEvent` 的 `payload` 都用它：
...
在基础命令方面，按照接口约定，终端至少应支持 `init`、`reset`、`start`、`stop`、`suspend`、`resume`。`init` 的参数需要和注册时声明的命令参数一致；`reset`、`start`、`stop` 同理。不过对于核心的仿真生命周期调度，我目前只把 `init`、`start`、`stop`、`reset` 纳入状态机。`suspend` 和 `resume` 作为辅助能力保留，后续如果调度流程需要再引入，不急于现在就把它们画进主状态图里。`Command` 里的 `timestamp` 是毫秒级 Unix 时间戳，终端可以用它来排序或去重乱序到达的命令。

我想在这里把命令和事件再区分清楚一些。命令是终端在任意状态下都会向外部提供的行为入口，它描述的是“这个终端能接收哪些控制动作”。这些能力通常在注册阶段通过 `TerminalType.supported_commands` 一次性声明，不会因为当前状态而改变。`init` 在没初始化时可以执行、`start` 在就绪后可以执行、`stop` 在运行后可以执行——这些是由状态机 guards 决定的，不是由注册信息动态增减的。换句话说，命令是稳定的控制接口，调度器可以预期某个终端一定支持哪些命令。

事件则不同。事件描述的是“在当前状态下，可以触发哪些行为”。同一个事件可能在某些状态下可用，在另一些状态下不可用；也可能随着仿真推进，出现新的可触发事件或让已有事件失效。因此事件能力不是注册阶段就能完全固定的，而是需要在运行过程中通过 `TerminalMetaStatus.publishable_events` 持续推送。不过要注意，这里的“可触发”是指控制系统可以向该终端发起触发请求，事件本身最终由控制系统（或管理面板/调度器）通过 `PublishEvent` 发起，终端并不自主产生事件。
...
```json
{
  ...
  "health": { "simple_value": { "int_value": 100 } },
  "sim_time_ms": { "simple_value": { "int_value": 120000 } },
  "frame": { "simple_value": { "int_value": 3600 } }
}
```

这些字段可以看作控制系统与模拟器终端之间的一套约定：`lifecycle_state` 表达当前处于哪个生命周期阶段，`health` 表达模拟器实体的“生命值”，用来判断实体是否仍然处于有效状态。对于非模拟器终端，这两个字段可以不存在，或者用其他字段表达各自的业务状态，这正是 `current_param` 保持通用的原因。

不过既然是约定，就需要在注册阶段也让控制系统知道。模拟器类终端在注册时，应该把 `lifecycle_state`、`health`、`sim_time_ms`、`frame` 这些标准字段声明在 `TerminalType.param_schema` 中，并标明它们分别是字符串、整数还是其他类型。这样管理面板不需要预先写死字段名，而是根据注册信息统一渲染状态；调度器也能在运行时发现某个终端缺少预期字段时，及时判定它不满足仿真流程的准入条件。

这种“约定加注册声明”的做法当然也有代价：字段名是否正确、类型是否匹配，没法靠 proto 编译器来保证，调度器读取状态时也要多做一次 map 查找和类型校验。所以自然会想到另一个方案——直接把 `lifecycle_state`、`health`、`sim_time_ms`、`frame` 提升为 `TerminalStatus` 的 `optional` proto 字段。这个方案的好处很直接：关键状态字段一目了然，调度代码可以 `status.lifecycle_state()` 直接访问，编码也更紧凑。

两种方式没有绝对的对错，只是取舍不同。我把它们放在一起对比：

|       维度       |                              放在 `current_param` 中（当前设计）                              |                                           提升为 `TerminalStatus` 的 `optional` 字段                                            |
| :--------------: | :-------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------------------------------: |
|      通用性      | 好。`TerminalStatus` 不绑定任何具体终端类型，雷达、数据记录、任务环境等非模拟器节点也能复用。 |            差。`TerminalStatus` 会带上模拟器专用语义，其他类型节点即便不需要这些字段，也要面对一份被污染的消息定义。            |
...
| 与注册信息的关联 |         自然。标准字段在 `TerminalType.param_schema` 里声明，管理面板按注册信息渲染。         | 变弱。这些字段变成隐式约定，除非在 `TerminalType` 里额外声明“本终端支持 lifecycle_state 等字段”，否则注册信息无法告诉控制系统。 |
...
综合看下来，我更看重左侧这些优势。因为这套控制系统中的终端是异构的：它后面可能是模拟器，也可能是雷达、任务环境、数据记录等其他类型的节点。如果把模拟器专用的标准字段固化到 `TerminalStatus` 里，其他类型终端就会面对一份被污染的消息；每新增一个标准字段，都要修改 proto 并重新发布接口，扩展性反而变差。所以我把它们留在 `current_param` 中，通过 `param_schema` 约定语义，而不是通过 proto 字段强制结构。
...
`TerminalMetaStatus` 的推送策略和 `TerminalStatus` 不同：它不是周期性推送，而是在内容发生变化时才推送。当终端允许被触发的事件集合发生变化，或者已触发事件实例增加或减少时，终端立即推送一次更新，保证管理面板能实时感知事件变化；没有变化时则不产生额外流量。高频的 `TerminalStatus` 推送频率由终端实现控制，目前约定最大不超过 100Hz。

### 一次典型控制流程

按照这个分工，我会把一次典型的控制流程分成注册、订阅、初始化、启动和状态反馈几个阶段。
...
sequenceDiagram
    ...
    C-->>T: TerminalRegisterResponse(success, message)
...
在这个流程里，`Center` 只解决“有哪些终端”和“它们的服务是否健康”的问题；`Terminal` 解决“如何控制具体模拟器”和“高频运行状态是什么”的问题；管理面板/调度器解决“在什么时机对哪些终端下发什么命令”的问题。状态路径不经过 Center 转发，调度器可以直接根据每个终端的状态流判断下一步动作。

### 状态迁移与确定性调度

为了保证调度确定性，我不希望控制系统只把命令当成普通 RPC 调用，而是要把每个命令都放进状态迁移中理解。`init`、`start`、`stop`、`reset` 等命令只有在特定状态下才允许执行，执行后也必须等待终端状态进入预期结果。这也是飞行仿真分布式系统和很多面向服务的分布式系统之间的一个重要区别：后者通常倾向于 **stateless** 的服务设计，而这里更接近 **stateful** 的流程控制。
...
stateDiagram-v2
    ...
    Initializing --> Disconnected: 通信中断
    Running --> Disconnected: 通信中断
    Disconnected --> Registered: 重新注册
```

在这个模型下，`TerminalStatus` 里的 `terminal_id` 让每条状态都可溯源；但它没有内置通用的“是否在线”字段，因为在线状态更适合由通信层和 `Center` 的服务健康状态来表达。`current_param` 里的 `lifecycle_state` 和 `health` 则负责表达业务层面的生命周期和实体状态。这样职责比较清晰：通信层负责“通不通”，服务健康负责“服务是否可用”，业务字段负责“处于哪个阶段、实体是否还存活”。一旦订阅流断开，调度器就知道这个终端从控制视角暂时不可达；如果终端重新注册到 Center，流程会回到 `Registered` 重新建立订阅。
...
特情类事件则完全不同。它应该由控制系统统一定义，并在初始化阶段通过任务配置同步到各个终端。这样特情的语义、参数结构和触发规则都由控制系统说了算，终端只负责在运行阶段接收和执行。具体流程可以分为两步：

1. **初始化阶段同步定义**：控制系统把本次任务涉及的特情事件清单及参数 schema 下发给相关终端，让终端知道“本次任务可能要处理哪些特情”。
2. **运行阶段精确触发**：控制系统根据训练脚本或人工指令，通过 `PublishEvent` 在精确的时机触发具体特情。
...
还需要区分的是**告警**。告警是终端根据自身状态自动产生的，例如发动机温度超限、液压压力不足等，终端发现后主动向控制系统上报。告警不属于事件机制，也不应该通过 `PublishEvent` 触发。事件是控制系统“让系统发生什么”，告警是终端“告诉系统发生了什么”，两者的发起方向和语义都不相同。

### 小结

总体来看，这套接口设计已经具备分布式仿真控制系统的基本骨架：`Center` 负责发现和健康监控，`Terminal` 负责适配、执行和状态推送，管理面板/调度器直接订阅终端状态并发起确定性控制流程。状态通道的拆分让高频运行状态和低频元信息各得其所，强类型的 `Parameter` 让命令和状态都可以被控制系统理解和校验，事件实例化的设计则让同类型事件的多次触发可以被精确管理。

不过真正影响系统可靠性的，还是把这些动态命令、动态状态和动态事件进一步纳入统一的状态模型和调度规则。只有当每一次仿真的推进路径都是可解释的、被追踪的、可复现的，这个控制系统才称得上稳定。
